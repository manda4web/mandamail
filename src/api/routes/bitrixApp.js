...
function getMappingPageHtml() {
  return [
    ...
    '    <h3>⚡ Comportamento</h3>',
    '    <p>Configure como o app trata emails recebidos e deals no Bitrix24.</p>',
    '    <div class="form-group">',
    '      <label>Modo de criação de Deals</label>',
    '      <select id="map-deal-mode" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:6px;font-size:14px">',
    '        <option value="create_new">Criar um deal novo para cada email</option>',
    '        <option value="merge_by_contact">Agrupar emails do mesmo remetente no mesmo deal</option>',
    '      </select>',
    '      <small style="color:#6b7280;font-size:11px;margin-top:4px;display:block">No modo "agrupar", se já existir um deal aberto para o contato, o email será vinculado ao deal existente.</small>',
    '    </div>',
    '    <div class="form-group">',
    '      <label>Sincronizar emails a partir de</label>',
    '      <input type="date" id="map-sync-date" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:6px;font-size:14px">',
    ...
    '      <p style="font-size:12px;color:#92400e;margin-top:4px">Deletar um deal no Bitrix24 NÃO afeta o monitoramento de emails. O IMAP continua funcionando independentemente. Contatos não são duplicados — o app sempre busca antes de criar.</p>',
    '    </div>',
    '  </div>',
    '',
    '  <div class="settings-section">',
    '    <h3>🔀 Pipeline e Estágio</h3>',
    '    <p>Selecione o pipeline (funil) e o estágio onde os deals serão criados automaticamente.</p>',
    '    <div class="form-group">',
    ...
    '    </div>',
    '    <div class="form-group">',
    '      <label>Estágio inicial do Deal</label>',
    ...
    '  <div class="settings-section">',
    '    <h3>👤 Responsável</h3>',
    '    <p>Quem será o responsável pelos deals criados a partir dos emails.</p>',
    '    <div class="form-group">',
    '      <label>Responsável pelo Deal</label>',
    ...
    '    <p>Configure como os dados do email são mapeados para os campos do deal no Bitrix24.</p>',
    '',
    ...
    '',
    '  <div class="settings-section">',
    '    <h3>🏷️ Origem do Lead/Deal</h3>',
    '    <p>Selecione a origem que será atribuída aos deals criados a partir dos emails.</p>',
    '    <div class="form-group">',
...
function getLogsPageHtml() {
  return [
    ...
    '  <div class="table-container">',
    '    <table>',
    '      <thead><tr><th>Data</th><th>De</th><th>Assunto</th><th>Conta</th><th>Status</th><th>Deal</th><th>Ações</th></tr></thead>',
...
function getAppJsLogs() {
  return [
    ...
    '        }',
    '        return "<tr><td>" + date + "</td><td>" + escHtml(ev.from_email || "—") + "</td><td>" + escHtml(ev.subject || "—") + "</td><td>" + escHtml(ev.account_email || "—") + "</td><td>" + badge + "</td><td>" + (ev.bitrix_deal_id || "—") + "</td><td>" + actions + "</td></tr>";',
    '      }).join("");',
    '    }',
    ...
    '  if (!currentTenantId) return;',
    '  if (!confirm("Reprocessar este email? Vai tentar criar o deal novamente.")) return;',
    ...
    '    if (result) {',
    '      html += \'<div style="margin-bottom:12px"><strong>Deal ID:</strong> \' + (result.bitrix_deal_id || "—") + "</div>";',
...
function getAppJsPlanAndUtils() {
  return [
    ...
    '      document.getElementById("map-sync-date").value = t.sync_start_date.substring(0, 10);',
    '    }',
    '  }).catch(function() {',
    '    loadStages("0");',
    '  });',
    ...
    '    sync_start_date: document.getElementById("map-sync-date").value ? new Date(document.getElementById("map-sync-date").value).toISOString() : null',
    '  }).then(function() {',
